"""Resume Generator — FastAPI backend."""
import json
...
from database import Generation, get_db, init_db
...
def generate(req: GenerateRequest):
    """Call the LLM and return structured JSON. Does NOT render PDF or save history."""
    resume_text = req.source_resume or load_default_resume_text()
    if not resume_text:
        raise HTTPException(400, "No source resume available. Provide source_resume or include backend/default_resume.txt.")
    try:
...
def render_pdf(req: RenderRequest, db: Session = Depends(get_db)):
    """Render PDF from (possibly user-edited) JSON. Optionally saves to history."""
